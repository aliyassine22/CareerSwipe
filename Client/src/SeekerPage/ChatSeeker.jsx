...
export default function ChatSeeker() {
  ...
  const fetchConversation = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:4000/seeker/messages/${companyId}`,
  ...
  return (
    <div className="p-6 flex flex-col h-full">
      <h2 className="text-2xl font-bold mb-4">Chat with Company</h2>
      <div className="flex-1 overflow-y-auto mb-4 space-y-4 flex flex-col">
        {messages.map(msg => (
          <div
            key={msg._id}
            className={`max-w-md p-3 rounded-lg ${
              msg.sender === 'JobSeeker' ? 'bg-green-100 self-end' : 'bg-gray-200 self-start'
            }`}
          >
